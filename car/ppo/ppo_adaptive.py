    ...
    for episode in range(EP_MAX + 1):
        ...
        while True:
            ...
            if (t > 0 and episode > 200):
                ...
                lambda_mix = lambda_max*(1 - np.exp(-factor*np.abs(r + GAMMA*np.squeeze(target_v) - np.squeeze(base_v))))
            else:
                lambda_mix = 10.
                lambda_actual = 10.
            ...
            if t == BATCH:  # or (terminal and t < BATCH):
                ...
                buffer_s, buffer_a, buffer_r, buffer_v, buffer_terminal = [], [], [], [], []
                t = 0
                lambda_actual = np.mean(lambda_store)
                print(np.mean(lambda_store))
