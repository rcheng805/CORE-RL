...
def train(sess, env, args, actor, critic, actor_noise, reward_result):
    ...
    for i in range(int(args['max_episodes'])):
        ...
        for j in range(int(args['max_episode_len'])):
            ...
            if (j > 0 and i > 50):
                s_old = np.reshape(s_old,((1,4)))
                s_new = np.reshape(s,((1,4)))
                # Obtain TD-error
                ...
                lambda_mix = lambda_max*(1 - np.exp(-factor*np.abs(r + critic.gamma * target_q - base_q)))
            else:
                lambda_mix = 10.
            lambda_store[j] = lambda_mix
            
            # Prior control with dynamic inversion term
...
if __name__ == '__main__':
    ...
    rand_seed = random.randrange(10000000)
